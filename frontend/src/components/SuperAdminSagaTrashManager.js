...
const SuperAdminSagaTrashManager = () => {
  ...
  const loadDeletedSagas = useCallback(async () => {
    if (!isSuperAdmin) return;

    try {
      const response = await api.get("/sagas/admin/deleted");
      ...
      setErrorMessage(error.response?.data?.error || "Impossible de récupérer les sagas en corbeille.");
    }
  }, [isSuperAdmin]);
  ...
  const handleRestore = async (saga) => {
    setSaving(true);
    resetFeedback();

    try {
      await api.put(`/sagas/${saga.SagaID}/restore`);
      await loadDeletedSagas();
      setMessage(`Saga restaurée : ${saga.Titre}`);
    } catch (error) {
      console.error("Erreur lors de la restauration :", error);
      setErrorMessage(error.response?.data?.error || "Impossible de restaurer cette saga.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handlePermanentDelete = async () => {
    ...
    try {
      await api.delete(`/sagas/${pendingDelete.SagaID}/permanent`);
      await loadDeletedSagas();
      setMessage(`Saga supprimée définitivement : ${pendingDelete.Titre}`);
      setPendingDelete(null);
    } catch (error) {
      console.error("Erreur lors de la suppression définitive :", error);
      setErrorMessage(error.response?.data?.error || "Impossible de supprimer définitivement cette saga.");
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <section className="mx-auto my-8 max-w-2xl overflow-hidden rounded-2xl border border-sky-500/10 bg-white/80 shadow-xl shadow-slate-950/5 backdrop-blur dark:bg-slate-950/70 dark:shadow-sky-950/20">
        <div className="border-b border-sky-500/10 bg-gradient-to-r from-sky-500/15 via-blue-500/10 to-transparent px-6 py-5">
          <h2 className="text-xl font-black text-slate-950 dark:text-white">Corbeille sagas</h2>
        </div>
  ...
  return (
    <section className="mx-auto my-8 max-w-4xl overflow-hidden rounded-2xl border border-sky-500/10 bg-white/80 shadow-xl shadow-slate-950/5 backdrop-blur dark:bg-slate-950/70 dark:shadow-sky-950/20">
      <div className="border-b border-sky-500/10 bg-gradient-to-r from-sky-500/15 via-blue-500/10 to-transparent px-6 py-5">
        ...
        <h2 className="mt-1 text-2xl font-black text-slate-950 dark:text-white">Corbeille sagas</h2>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-300">
          Restaure une saga ou supprime définitivement ses liaisons et son dossier d'affiche.
        </p>
      </div>
      ...
      <div className="relative px-6 py-6">
        ...
        <div className="relative">
          ...
          <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            />
            <button type="button" onClick={loadDeletedSagas} disabled={saving} className={buttonClass}>
          ...
          {loadingAuth ? (
            ...
            <p className="rounded-xl border border-sky-500/10 bg-white/70 px-4 py-5 text-sm font-semibold text-slate-600 dark:bg-slate-950/40 dark:text-slate-300">Aucune saga en corbeille.</p>
          ) : (
            <ul className="divide-y divide-sky-500/10 overflow-hidden rounded-xl border border-sky-500/10 bg-white/70 dark:bg-slate-950/40">
              {filteredSagas.map((saga) => (
                <li key={saga.SagaID} className="flex flex-col gap-4 px-4 py-4 sm:flex-row sm:items-center">
                  <div className="flex flex-1 items-center gap-4">
                    <div className="h-20 w-14 shrink-0 overflow-hidden rounded-lg bg-slate-200 dark:bg-slate-800">
                      {saga.CheminImage && <img src={`${apiUrl}/${saga.CheminImage}`} alt="" className="h-full w-full object-cover" />}
                    </div>
                    <div>
                      <p className="font-black text-slate-950 dark:text-white">{saga.Titre}</p>
                      <p className="mt-1 text-xs font-semibold uppercase text-slate-500 dark:text-slate-400">#{saga.SagaID}</p>
                    </div>
                  </div>
                  <div className="flex flex-col gap-2 sm:flex-row">
                    <button type="button" onClick={() => handleRestore(saga)} disabled={saving} className={buttonClass}>Restaurer</button>
                    <button type="button" onClick={() => setPendingDelete(saga)} disabled={saving} className={dangerButtonClass}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 px-4">
          <div className="w-full max-w-md rounded-2xl border border-red-300/20 bg-white p-6 shadow-2xl dark:bg-slate-950 dark:text-white">
            ...
            <p className="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-300">
              Supprimer définitivement "{pendingDelete.Titre}" ? Cette action retire les liaisons, supprime la saga et son dossier d'affiche. Les contenus liés ne seront pas supprimés.
            </p>
...
export default SuperAdminSagaTrashManager;
