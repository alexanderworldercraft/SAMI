...
export const addVideo = async (req, reply, fastify) => {
  try {
    ...
    try {
      const previewActive = await isContentPreviewActive();
      if (previewActive) {
        const previewFrames = await generateVideoPreviewFramesFromMaster({
          videoId: video.VideoID,
          masterPlaylistPath: path.join(finalHlsDir, "master.m3u8"),
        });
        console.log(`[addVideo] ${previewFrames.length} images de prévisualisation générées pour la vidéo ${video.VideoID}.`);
      }
    } catch (previewError) {
      console.warn(
        `[addVideo] Prévisualisation non générée pour la vidéo ${video.VideoID} :`,
